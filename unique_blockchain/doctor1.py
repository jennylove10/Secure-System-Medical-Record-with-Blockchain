...
def run():
    ...
    with grpc.insecure_channel('127.0.0.1:50090') as channel:
        ...
        response = stub.diagnosis(blockchain_pb2.diagnosisRequest(doctorname=doctorname, patientname=patientname ,prescription= prescription,fee= fee))
        if response :
            print(doctorname + ' made a diagnosis failed because he is not authorized')
        else:
